...
func replaceProductName(text string) string {
	var re = regexp.MustCompile(`(^|\s)(Cortex)(\.|\s|$)`)
	return re.ReplaceAllString(text, `${1}Loki${3}`)
}
...
func (w *specWriter) writeConfigEntry(e *configEntry, indent int) {
	if e.kind == "block" {
		...
		if e.root {
			// Description
			w.writeComment(replaceProductName(e.blockDesc), indent)
	...
	if e.kind == "field" {
		// Description
		w.writeComment(replaceProductName(e.fieldDesc), indent)
