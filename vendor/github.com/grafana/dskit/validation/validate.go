...
import (
	...
	"github.com/grafana/dskit/labels"
)
...
func ValidateSample(cfg SampleValidationConfig, userID string, ls []dskitpb.LabelAdapter, s dskitpb.Sample) ValidationError {
...
// The returned error may retain the provided series labels.
func ValidateExemplar(userID string, ls []dskitpb.LabelAdapter, e dskitpb.Exemplar) ValidationError {
	if len(e.Labels) <= 0 {
		DiscardedExemplars.WithLabelValues(exemplarLabelsMissing, userID).Inc()
		return newExemplarEmtpyLabelsError(ls, []dskitpb.LabelAdapter{}, e.TimestampMs)
	}
...
// The returned error may retain the provided series labels.
func ValidateLabels(cfg LabelValidationConfig, userID string, ls []dskitpb.LabelAdapter, skipLabelNameValidation bool) ValidationError {
...
func ValidateMetadata(cfg MetadataValidationConfig, userID string, metadata *dskitpb.MetricMetadata) error {
