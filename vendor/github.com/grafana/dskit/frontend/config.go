...
func InitFrontend(cfg CombinedFrontendConfig, limits v1.Limits, grpcListenPort int, logger log.Logger, reg prometheus.Registerer) (http.RoundTripper, *v1.Frontend, *v2.Frontend, error) {
	switch {
	...
	case cfg.FrontendV2.SchedulerAddress != "":
		...
		if cfg.FrontendV2.Addr == "" {
			addr, err := netutil.GetFirstAddressOf(cfg.FrontendV2.InfNames, logger)
			if err != nil {
		...
		fr, err := v2.NewFrontend(cfg.FrontendV2, logger, reg)
		return transport.AdaptGrpcRoundTripperToHTTPRoundTripper(fr), nil, fr, err

	default:
		// No scheduler = use original frontend.
		fr, err := v1.New(cfg.FrontendV1, limits, logger, reg)
		if err != nil {
