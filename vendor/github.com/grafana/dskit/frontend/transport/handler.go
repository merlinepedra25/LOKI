...
import (
	...
	"github.com/grafana/dskit/cleanupusers"
	"github.com/grafana/dskit/dshttp"
	"github.com/grafana/dskit/dslog"
	querier_stats "github.com/grafana/dskit/querier/stats"
...
type Handler struct {
	...
	activeUsers  *cleanupusers.ActiveUsersCleanupService
}
...
func NewHandler(cfg HandlerConfig, roundTripper http.RoundTripper, log log.Logger, reg prometheus.Registerer) http.Handler {
	...
	if cfg.QueryStatsEnabled {
		...
		h.activeUsers = cleanupusers.NewActiveUsersCleanupWithDefaultValues(func(user string) {
...
func (f *Handler) reportSlowQuery(r *http.Request, queryString url.Values, queryResponseTime time.Duration) {
	...
	level.Info(dslog.WithContext(r.Context(), f.log)).Log(logMessage...)
}
...
func (f *Handler) reportQueryStats(r *http.Request, queryString url.Values, queryResponseTime time.Duration, stats *querier_stats.Stats) {
	tenantIDs, err := tenant.IDs(r.Context())
	if err != nil {
		return
	}
...
func (f *Handler) parseRequestQueryString(r *http.Request, bodyBuf bytes.Buffer) url.Values {
	...
	err := r.ParseForm()
	if err != nil {
		level.Warn(dslog.WithContext(r.Context(), f.log)).Log("msg", "unable to parse request form", "err", err)
		return nil
	}
...
func writeError(w http.ResponseWriter, err error) {
	...
	default:
		if dshttp.IsRequestBodyTooLarge(err) {
