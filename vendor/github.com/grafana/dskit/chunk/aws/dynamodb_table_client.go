...
func NewDynamoDBTableClient(cfg DynamoDBConfig, reg prometheus.Registerer, logger log.Logger) (chunk.TableClient, error) {
	...
	if cfg.Metrics.URL != "" {
		autoscale, err = newMetricsAutoScaling(cfg, logger)
		if err != nil {
			return nil, err
		}
	}
	...
	for backoff.Ongoing() {
		if err := fn(ctx); err != nil {
			if awsErr, ok := err.(awserr.Error); ok && awsErr.Code() == "ThrottlingException" {
				level.Warn(dslog.WithContext(ctx, d.logger)).Log("msg", "got error, backing off and retrying", "err", err, "retry", backoff.NumRetries())
				backoff.Wait()
				continue
			} else {
				return err
...
func (d dynamoTableClient) UpdateTable(ctx context.Context, current, expected chunk.TableDesc) error {
	...
	level.Debug(d.logger).Log("msg", "Updating Table",
	...
	if (current.ProvisionedRead != expected.ProvisionedRead ||
		...
		level.Info(d.logger).Log("msg", "updating provisioned throughput on table", "table", expected.Name, "old_read", current.ProvisionedRead, "old_write", current.ProvisionedWrite, "new_read", expected.ProvisionedRead, "new_write", expected.ProvisionedWrite)
		if err := d.backoffAndRetry(ctx, func(ctx context.Context) error {
			return instrument.CollectedRequest(ctx, "DynamoDB.UpdateTable", d.metrics.dynamoRequestDuration, instrument.ErrorCode, func(ctx context.Context) error {
				...
				if current.UseOnDemandIOMode != expected.UseOnDemandIOMode {
					...
					level.Info(d.logger).Log("msg", "updating billing mode on table", "table", expected.Name, "old_mode", current.UseOnDemandIOMode, "new_mode", expected.UseOnDemandIOMode)
			...
			if awsErr, ok := err.(awserr.Error); ok && awsErr.Code() == "LimitExceededException" {
				level.Warn(d.logger).Log("msg", "update limit exceeded", "err", err)
			} else {
				return err
			}
		}
