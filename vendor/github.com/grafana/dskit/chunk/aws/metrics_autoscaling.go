...
func scaleDown(tableName string, currentValue, minValue int64, newValue int64, lastUpdated map[string]time.Time, coolDown int64, msg, operation string, usageRates map[string]float64, logger log.Logger) int64 {
	...
	if earliest.After(mtime.Now()) {
		level.Info(logger).Log("msg", "deferring "+msg, "table", tableName, "till", earliest, "op", operation)
		return currentValue
	}
	...
	if newValue > currentValue*4/5 {
		level.Info(logger).Log("msg", "rejected de minimis "+msg, "table", tableName, "current", currentValue, "proposed", newValue, "op", operation)
		return currentValue
	}
	...
	if usageRates != nil {
		...
		if totalUsage < minUsageForScaledown {
			level.Info(logger).Log("msg", "rejected low usage "+msg, "table", tableName, "totalUsage", totalUsage, "op", operation)
			return currentValue
		}
	}

	level.Info(logger).Log("msg", msg, "table", tableName, operation, newValue)
...
func scaleUp(tableName string, currentValue, maxValue int64, newValue int64, lastUpdated map[string]time.Time, coolDown int64, msg, operation string, logger log.Logger) int64 {
	...
	level.Info(logger).Log("msg", "deferring "+msg, "table", tableName, "till", earliest)
	return currentValue
}
