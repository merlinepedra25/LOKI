...
import (
	...
	"github.com/grafana/dskit/dskitpb" //lint:ignore faillint allowed to import other protobuf
)
...
func formattedRuleToProto(rls []rulefmt.RuleNode) []*RuleDesc {
	...
	for i := range rls {
		rules[i] = &RuleDesc{
			...
			Labels:      dskitpb.FromLabelsToLabelAdapters(labels.FromMap(rls[i].Labels)),
			Annotations: dskitpb.FromLabelsToLabelAdapters(labels.FromMap(rls[i].Annotations)),
		}
	}
...
func FromProto(rg *RuleGroupDesc) rulefmt.RuleGroup {
	...
	for i, rl := range rg.GetRules() {
		...
		newRule := rulefmt.RuleNode{
			Expr:        exprNode,
			Labels:      dskitpb.FromLabelAdaptersToLabels(rl.Labels).Map(),
			Annotations: dskitpb.FromLabelAdaptersToLabels(rl.Annotations).Map(),
