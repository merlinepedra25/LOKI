...
func FromLabelAdaptersToMetric(labels []LabelAdapter) model.Metric {
	ls := FromLabelAdaptersToLabels(labels)
...
func SampleJsoniterDecode(ptr unsafe.Pointer, iter *jsoniter.Iterator) {
	if !iter.ReadArray() {
		iter.ReportError("dskitpb.Sample", "expected [")
		return
	}
	...
	if err != nil {
		iter.ReportError("dskitpb.Sample", err.Error())
		return
	}
...
func init() {
	jsoniter.RegisterTypeEncoderFunc("dskitpb.Sample", SampleJsoniterEncode, func(unsafe.Pointer) bool { return false })
	jsoniter.RegisterTypeDecoderFunc("dskitpb.Sample", SampleJsoniterDecode)
}
