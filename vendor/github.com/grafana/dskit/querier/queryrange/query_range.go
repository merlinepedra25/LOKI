...
func (prometheusCodec) DecodeRequest(_ context.Context, r *http.Request) (Request, error) {
	...
	var err error
	result.Start, err = timeutil.ParseTime(r.FormValue("start"))
	if err != nil {
		return nil, decorateWithParamName(err, "start")
	}

	result.End, err = timeutil.ParseTime(r.FormValue("end"))
	if err != nil {
...
func (s *SampleStream) UnmarshalJSON(data []byte) error {
	var stream struct {
		...
		Values []dskitpb.Sample `json:"values"`
	}
	if err := json.Unmarshal(data, &stream); err != nil {
		return err
	}
	s.Labels = dskitpb.FromMetricsToLabelAdapters(stream.Metric)
...
func (s *SampleStream) MarshalJSON() ([]byte, error) {
	stream := struct {
		...
		Metric: dskitpb.FromLabelAdaptersToMetric(s.Labels),
		Values: s.Samples,
	}
...
func matrixMerge(resps []*PrometheusResponse) []SampleStream {
	...
	for _, resp := range resps {
		for _, stream := range resp.Data.Result {
			metric := dskitpb.FromLabelAdaptersToLabels(stream.Labels).String()
...
func sliceSamples(samples []dskitpb.Sample, minTs int64) []dskitpb.Sample {
