...
func (l limitsMiddleware) Do(ctx context.Context, r Request) (Response, error) {
	...
	*/
	...
	if maxQueryLookback := validation.SmallestPositiveNonZeroDurationPerTenant(tenantIDs, l.MaxQueryLookback); maxQueryLookback > 0 {
		minStartTime := timeutil.TimeToMillis(time.Now().Add(-maxQueryLookback))
		...
		if r.GetEnd() < minStartTime {
			...
			/*
				level.Debug(log).Log(
					...
					"reqStart", timeutil.FormatTimeMillis(r.GetStart()),
					"redEnd", timeutil.FormatTimeMillis(r.GetEnd()),
		...
		if r.GetStart() < minStartTime {
			...
			/*
				level.Debug(log).Log(
					...
					"original", timeutil.FormatTimeMillis(r.GetStart()),
					"updated", timeutil.FormatTimeMillis(minStartTime))
			*/
