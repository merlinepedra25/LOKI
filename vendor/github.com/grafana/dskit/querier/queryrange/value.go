...
func mapLabels(ls labels.Labels) []dskitpb.LabelAdapter {
	result := make([]dskitpb.LabelAdapter, 0, len(ls))
	for _, l := range ls {
		result = append(result, dskitpb.LabelAdapter(l))
	}

	return result
}

func mapPoints(pts ...promql.Point) []dskitpb.Sample {
	result := make([]dskitpb.Sample, 0, len(pts))

	for _, pt := range pts {
		result = append(result, dskitpb.Sample{
			Value:       pt.V,
			TimestampMs: pt.T,
		})
	}
